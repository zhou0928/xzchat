...
export function initConfigFile(customPath) {
  const targetFile = customPath || homeConfigFile;
  ...
  logger.info(`✅ 已创建配置文件: ${targetFile}`);
  logger.warn("⚠️  请务必编辑该文件，填入你的 apiKey (sk-...)，否则无法使用。");
}

export function initProjectConfigFile(customPath) {
  const targetFile = customPath || path.join(process.cwd(), ".newapi-chat-config.json");
...
export function loadConfig(customConfigPath) {
  // 如果指定了自定义配置路径，只从该路径加载
  if (customConfigPath) {
    return loadConfigFrom(customConfigPath);
  }
...
export function getWriteConfigFile(customPath) {
...
export function updateConfig(key, value, customPath) {
  const target = getWriteConfigFile(customPath);
...
export function setProfileValue(pathOrProfileName, keyOrValue, value, customPath) {
  const target = getWriteConfigFile(customPath);
  let current = loadConfigFrom(target);
  ...
  // 判断是哪种 API
  if (value !== undefined) {
    // 旧 API: setProfileValue(profileName, key, value)
    const profileName = pathOrProfileName;
    ...
    // 新 API: setProfileValue('profiles.dev.model', value)
    const path = pathOrProfileName;
    const val = keyOrValue;
    
    // 解析路径 "profiles.dev.model"
    const keys = path.split('.');
    
    if (keys[0] === 'profiles' && keys.length >= 3) {
      const profileName = keys[1];
      const nestedPath = keys.slice(2);
      
      if (!current.profiles[profileName]) current.profiles[profileName] = {};
      
      // 设置嵌套值
      let obj = current.profiles[profileName];
      for (let i = 0; i < nestedPath.length - 1; i++) {
        if (!obj[nestedPath[i]]) obj[nestedPath[i]] = {};
        obj = obj[nestedPath[i]];
      }
      obj[nestedPath[nestedPath.length - 1]] = val;
    }
  }
...
export function getActiveConfig(configOrProfileName, customConfigPath) {
  let config, profileName;

  // 判断参数类型
  if (typeof configOrProfileName === 'string' || configOrProfileName === undefined) {
    // 新 API: getActiveConfig(profileName?, customConfigPath?)
    config = loadConfig(customConfigPath);
    profileName = configOrProfileName || config.currentProfile || "default";
  } else {
    // 旧 API: getActiveConfig(config, customConfigPath?)
    config = configOrProfileName;
    profileName = config.currentProfile || "default";
  }

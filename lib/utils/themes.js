...
/**
 * Colorize 对象 - 提供颜色格式化方法
 * 兼容旧的 colorize API
 */
export const colorize = {
  success: (message) => getThemeInstance().formatSuccess(message),
  error: (message) => getThemeInstance().formatError(message),
  warning: (message) => getThemeInstance().formatWarning(message),
  info: (message) => getThemeInstance().formatInfo(message),
  command: (message) => getThemeInstance().formatCommand(message),
  code: (message) => getThemeInstance().formatCode(message),
  path: (message) => getThemeInstance().formatPath(message),
  header: (message) => `\n${'='.repeat(50)}\n${message}\n${'='.repeat(50)}\n`,
  emphasis: (message) => getThemeInstance().formatEmphasis(message)
};

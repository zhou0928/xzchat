...
export class PluginValidator {
  ...
  validateSecurity(plugin) {
    ...
    // 检查是否使用了危险的 eval
    if (plugin.instance) {
      const code = plugin.instance.toString();
    ...
    // 检查网络请求
    if (plugin.instance && plugin.instance.toString().includes('fetch(')) {
      if (!plugin.metadata.permissions?.includes('network')) {
        warnings.push({
          type: 'SECURITY',
          severity: 'warning',
    ...
    // 检查文件操作
    if (plugin.instance && plugin.instance.toString().includes('fs.')) {
      if (!plugin.metadata.permissions?.includes('file')) {
        warnings.push({
          type: 'SECURITY',
          severity: 'warning',
    ...
    if (plugin.instance) {
      ...
      if (code.includes('fs.readFileSync') || code.includes('fs.writeFileSync')) {
        warnings.push({
          type: 'PERFORMANCE',
          severity: 'warning',
          message: 'Plugin uses synchronous file operations, consider using async versions'
        });
      }

      // 检查同步网络请求
      if (code.includes('https.request') && !code.includes('await')) {
        warnings.push({
          type: 'PERFORMANCE',
          severity: 'warning',
          message: 'Network requests should be asynchronous'
        });
      }
    }
